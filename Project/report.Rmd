...
Since we don't have any information about movements or death or birth during the study period, the assumption of a closed population cannot be confirmed in this experiment. Therefore some open population models are also run, where model types as $M_0$ and $M_t$ can also be implemented. To estimate the super-population as well as the population size that is on the patch on every session, the first open model to run is the POPAN model, which is a modified parameterizantion of the Jolly-Seber model. This model not only estimates the population size, but also the migration or death and immigration or birth for every session. Additional parameters of these models are the survival probability (where emigration is included because it is indistinguishable) and the probability that an animal would enter the population (where birth is included) from one session to the next. 
To further analyse the population dynamics, a second open model, the Pradel-Lambda-model (PradLambda) is run. This model does not only give information about the population size, but how many individuals are new to the population on each session and how many had disappeared. It models the rate of population growth $\lambda$. Compared to the POPAN model it is conditional upon animals been seen during the study.
...
Open models...
The POPAN model that fits best is the time-dependent model...
...
openres$mPOPAN2$AICc <- mPOPAN2$results$AICc
## open with time
...
par(mfrow = c(1,2))
boxplot(
  t(mPOPAN$results$derived$`N Population Size`), 
  main = "Popan-model M_0", xlab = "Session", ylab = "N Population Size"
)
boxplot(
  t(mPOPANt$results$derived$`N Population Size`), 
  main = "Popan-model M_t", xlab = "Session", ylab = "N Population Size"
)
par(mfrow = c(1,1))
